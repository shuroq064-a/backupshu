...
def booking_agent_prompt() -> str:
    return (
        ...
        "names, prices, or ETAs beyond what the tools returned. ETA RULE (CRITICAL): arrival "
        "times are PER-SPECIALIST and appear in the tool data as that specialist's own "
        "'~X min to reach you' value. Never state one ETA for all specialists, never round or "
        "reuse one specialist's ETA for another, and never quote an ETA the tool data does not "
        "show for that exact specialist. If a specialist has no ETA in the tool data, do not "
        "invent one — just mention their distance. If no specialists were found but "
